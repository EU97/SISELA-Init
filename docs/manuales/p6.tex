...
5 & Registro CSV & Barrido con muestreo de ADC; CSV \code{t\_us,duty\_pct,adc\_raw} \\
\bottomrule
\end{tabular}
\end{table}
Tecla \code{m} $\to$ regresar al menú en cualquier modo.

\paragraph{Modo 5 — recolección de datos y vista en vivo.} Alimenta la toolkit
compartida \code{tools/sisela\_signal/} (disponible en MicroPython y C++, mismo
formato CSV):
...
El archivo \code{p6.cpp} implementa cuatro modos usando la clase
\code{PropulsionSystem} (\code{common/propulsion.h}, API en porcentaje 0--100):
rampa, control por serial, control por ADC, y un cuarto modo de registro CSV
(\code{siglab::serialPrintf}, \code{common/siglab.h}) equivalente al Modo 5 de
MicroPython — mismo formato \code{t\_us,duty\_pct,adc\_raw}, compatible con la
misma toolkit \code{tools/sisela\_signal}. Compilar con \code{pio run -e
esp32dev} o \code{pio run -e pico} (con \code{-DPRACTICE=6}).
