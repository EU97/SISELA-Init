"""
Programa principal (main.py) — RP2040 + MicroPython
...
def menu_select(timeout_s=8):
    ...
    print("1) Blink LED1 (GP25 - onboard)")
...
def blink_led1(period_s=1.0):
    print("[modo 1] Blink LED1 en GP{}".format(LED1_PIN))
    poll = _new_poll()
    state = False
    try:
        while True:
...
def chaser(period_s=0.3):
    print("[modo 2] Secuencia en 3 LEDs: GP{}, GP{}, GP{}".format(LED1_PIN, LED2_PIN, LED3_PIN))
    poll = _new_poll()
    leds = [led1, led2, led3]
    idx = 0
    try:
...
def monitor_inputs(sample_ms=200):
    print("[modo 3] Monitor de entradas BTN1=GP{} BTN2=GP{} (pull-up, activo LOW)".format(BTN1_PIN, BTN2_PIN))
    poll = _new_poll()
    try:
        while True:
            if check_menu_break(poll):
...
def main():
    print("[main] RP2040 listo. Escribe 'm' en cualquier modo para volver al menú.")
    while True:
        sel = menu_select()
        if sel == 1:
            blink_led1()
